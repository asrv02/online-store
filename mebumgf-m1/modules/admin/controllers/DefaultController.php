...
*/
        ...
        if( $id = Yii::$app->request->get('id')){
            if($model = $this->findModel($id)){
                if($this->request->isPost && $model->load($this->request->post())){
                    $model->status_id = Status::findOne(['title' => 'Отменённый'])->id;
                    $model->save();
    ...
    public function actionApplyOrder($id)
    {
        ...
        if($model = $this->findModel($id)){
            $model->status_id = Status::findOne(['title' => 'Подтверждённый'])->id;
            $model->save();
            Yii::$app->session->setFlash('check1', 1);
            return $this->redirect('index');
    ...
    public function actionCancelOrder($id)
    {
        ...
        if($model = $this->findModel($id)){
            // $model->status_id = Status::findOne(['title' => 'Отменённый'])->id;
            // $model->save();
            // Yii::$app->session->setFlash('check2', 1);
            // return $this->redirect('index');
            $searchModel = new AdminSearch();
            $dataProvider = $searchModel->search($this->request->queryParams);
            $status = Status::getStatusList();
